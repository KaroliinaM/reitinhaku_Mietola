...
public class PerformanceTest {
    ...
    public void run() {
        ...
        for (int i = 0; i < routesTotest; i++) {
            ...
            dijkstrafinder.setMapdata(mapdata);
             ...
             if(!route.getIfExists()) continue;
            double distance=calculator.distFrom(beg.getLat(), beg.getLon(), end.getLat(), end.getLon());
            if(distance<10000) {
               if(pointer10>29) continue;
               under10starts[pointer10] = beg;
               under10goals[pointer10] = end;
               pointer10++;
            } else if(distance<20000) {
               if(pointer20>29) continue;
               under20starts[pointer20] = beg;
               under20goals[pointer20] = end;
               pointer20++;
            } else if(distance<30000) {
               if(pointer30>29) continue;
               under30starts[pointer30] = beg;
               under30goals[pointer30] = end;
               pointer30++;
            } else {
                if(pointer30ab>29) continue;
               above30starts[pointer30ab] = beg;
               above30goals[pointer30ab] = end;
               pointer30ab++;
            }
        }
        System.out.println("under 10 " + pointer10 + "under 20 "+ pointer20+"under 30 " +pointer30+"above 30 "+ pointer30ab);
       testDistance(pointer10, under10starts, under10goals, dijkstraresult, astarresult); 


    }
    public void testDistance(int pointer, Stop[] starts, Stop[] goals, String[] dijkstraresult, String[] astarresult) {
    ...
    public String getAstarRouteTime(Stop start, Stop end) {
           ...
           OptimalRoute route=null;
            ...
            long time = (t / timesForRoute) / 3600;
            String thisResult = route.getIfExists()+"astarilla haku pysäkiltä " + start.getName()+start.getGtfsId()
